...
but non-basic interfaces may only be used as type constraints (as of Go 1.23).
...
Currently (Go 1.23), the `comparable` interface is treated as a non-basic interface type.
...
As of Go 1.23, type satisfactions are used to verify whether or not an ordinary value type
...
For example, the following term unions are both illegal (as of Go toolchain 1.23):
...
In fact, currently (Go 1.23), [type parameters may not be embedded in struct types](888-the-status-quo-of-Go-custom-generics.md#embed-type-parameter), too.
